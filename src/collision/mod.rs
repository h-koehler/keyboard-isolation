...
pub fn collision_detection_system(
    ...
    if let Ok((mut player_transform, player_collider, mut player_velocity)) = player_query.single_mut() {
        ...
        for (object_transform, object_collider) in object_query.iter() {
            let object_collision_pos = object_collider.collision_position(object_transform.translation);
    ...
    for placement in placements {
            ...
            (placement.collision_size, placement.collision_offset) 
        {
            ...
            commands.spawn(WorldObjectBundle {
                world_object: WorldObject { object_type: placement.object_type },
...
pub(super) fn register(app: &mut App) {
    // Register y-sorting first
    crate::y_sort::register(app);
    
    // Then collision detection (after velocity, before camera)
    app.add_systems(PostUpdate, collision_detection_system.after(crate::character_controls::apply_velocity));
}